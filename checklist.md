...
|4|フォントが合っていること（`font-family`）<br><small>※ 検証ツールの[要素]-[計算済み]-[レンダリングフォント]で確認</small>|NG|2026/5/5|コピーライト（フッター）のフォントがデザインカンプ通りでない。|
...
|11|Tabキー操作でフォーカス中の要素が分かりやすく表示されること|NG|2026/5/5|ハンバーガーメニューアイコンのフォーカス状態が分かりにくい。|
...
|2|500KBを超えるリソースが無いこと<br><small>※1 検証ツールのNetworkタブで確認</small><br><small>※2 PC/SPそれぞれで確認（画像出し分けを考慮）</small>|OK|2026/5/5||
|3|横スクロールしないこと|OK|2026/5/5||
|4|フォントが合っていること（`font-family`）<br><small>※ 検証ツールの[要素]-[計算済み]-[レンダリングフォント]で確認</small>|NG|2026/5/5|パンくずリストのフォントがデザインカンプ通りでない。|
|5|フォントの色が適切であること（`color`）|OK|2026/5/5||
|6|フォントサイズが適切であること（`font-size`）|OK|2026/5/5||
|7|文章の行間が適切であること（`line-height`）|OK|2026/5/5||
|8|余白が適切であること（`margin`/`padding`）|OK|2026/5/5||
...
|11|Tabキー操作でフォーカス中の要素が分かりやすく表示されること|NG|2026/5/5|カテゴリータブがフォーカス中のときに疑似要素とテキストの高さがずれる。|
|12|フォームの必須項目がすべて入力されるまで送信できないこと|N/A|N/A|フォームが無いため|
|13|フォームの入力内容が適切に送信されること|N/A|N/A|フォームが無いため|
|14|ハンバーガーメニューが見切れた時に縦スクロールできること|OK|2026/5/5||
...
|4|フォントが合っていること（`font-family`）<br><small>※ 検証ツールの[要素]-[計算済み]-[レンダリングフォント]で確認</small>|NG|2026/5/9|パンくずリストのフォントがデザインカンプ通りでない。|
...
|11|Tabキー操作でフォーカス中の要素が分かりやすく表示されること|NG|2026/5/9|ハンバーガーメニューアイコンのフォーカス状態が分かりにくい。|
|12|フォームの必須項目がすべて入力されるまで送信できないこと|N/A|N/A|フォームが無いため|
|13|フォームの入力内容が適切に送信されること|N/A|N/A|フォームが無いため|
...
|4|フォントが合っていること（`font-family`）<br><small>※ 検証ツールの[要素]-[計算済み]-[レンダリングフォント]で確認</small>|NG|2026/5/10|パンくずリストのフォントがデザインカンプ通りでない。|
|5|フォントの色が適切であること（`color`）|OK|2026/5/10||
|6|フォントサイズが適切であること（`font-size`）|OK|2026/5/10||
|7|文章の行間が適切であること（`line-height`）|NG|2026/5/10|リストの行間が広すぎる。|
...
|11|Tabキー操作でフォーカス中の要素が分かりやすく表示されること|NG|2026/5/10|ハンバーガーメニューアイコンのフォーカス状態が分かりにくい。|
|12|フォームの必須項目がすべて入力されるまで送信できないこと|N/A|N/A|フォームが無いため|
|13|フォームの入力内容が適切に送信されること|N/A|N/A|フォームが無いため|
...
|11|Tabキー操作でフォーカス中の要素が分かりやすく表示されること|NG|2026/5/11|TOPへ戻るボタンにフォーカスが当たると、矢印アイコンが右に移動する。|
|12|フォームの必須項目がすべて入力されるまで送信できないこと|N/A|N/A|フォームが無いため|
|13|フォームの入力内容が適切に送信されること|N/A|N/A|フォームが無いため|
